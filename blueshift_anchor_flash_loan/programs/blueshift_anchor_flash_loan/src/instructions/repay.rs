...
impl<'info> Repay<'info> {
    fn refund_to_protocol(&mut self) -> Result<()> {
        ...
        if let Ok(borrow_ixs) = load_instruction_at_checked(0, &ixs) {
            ...
            // create an empty container of 8 bytes
            let mut borrowed_data: [u8; 8] = [0u8; 8];
            //copy bytes 8-16 from instruction data to container
            borrowed_data.copy_from_slice(&borrow_ixs.data[8..16]);
            // Convert those 8 bytes into an actual number (u64)
        ...
        // fee = amount × BASIS_POINTS / 10_000 (1/100(1 percent) so 100/10000)
        let fee = (amount_borrowed as u128)
            .checked_mul(500) //safe math operations in Rust that prevent overflow/underflow bugs.(chewcked multiply)
            .unwrap()
            .checked_div(10_000)
        ...
        // Transfer the funds from borrower to protocol
        transfer(
            CpiContext::new(
